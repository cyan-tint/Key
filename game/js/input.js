...
window.Input = (function () {
  ...
  /**
   * new Input.DragController({
   *   handle:        HTMLElement,  // 触发拖拽的元素（mousedown 起点）
   *   target:        HTMLElement,  // 被拖动时跟随指针的元素（默认同 handle）
   *   moveElement:   boolean,      // 是否让 target 跟随指针移动（默认 true）
   *   hitTargets:    HTMLElement[],// 落下时检测命中的目标列表
   *   onStart(x,y,ev),
   *   onMove(x,y,ev,state),        // state: {dx, dy, overTarget}
   *   onDrop(x,y,ev,hitEl|null),   // 落点命中某个 hitTarget 时为该元素，否则 null
   *   onHit(hitEl)                 // 等价于 onDrop 命中时
   * })
   */
  ...
  DragController.prototype._hitTest = function (x, y) {
    const targets = this.cfg.hitTargets || [];
